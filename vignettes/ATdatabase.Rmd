...
Data from a single environemental sensor are usually send to data
...
measurements are stored as structured data, meta data about the
station
or sensor (or project or region where they belong to) can be stored in any format, as unstructured data. 
For the storage of these meta data a NoSQL approach is used.
In this approach meta data is stored as JSON documents.
...
The station table contains basic information about the sensor, like
name and position.
...
the `insert_sensor_info` function from the package.

```{r}

insert_sensor_info_vectorized <- function(x, conn) {

    insert_sensor_info(station = x[1],
                        lat = as.numeric(x[2]),
                        lon = as.numeric(x[3]),
                        conn)
...
apply(stat_info, 1, FUN  = insert_sensor_info_vectorized, 
      conn = dbconn)

```

Now check how our sensor table looks like.
...
insert_station_info function adds the current timestamp to the record
