...
Let's start with the arguments of the function. The timeranges is a
matrix with a single row and two columns. The first column is the start time in
seconds (POSIX time) and the second column in the end time in seconds.
Using the lubridate package you can translate this 'time in seconds'
into any date format as needed.
...
following fields:
 ...
 5. _aggregation_ aggregation level

## Example data

For the sake of simplicity, for this vignette we use a file with
measurements instead of an database or API. Our download handler this
...
time_stamp <- matrix(c(start_date, end_date), ncol = 2)
time_stamp
